...
module tb_uart;
    ...
    // RX信号
    logic recv_vld;
    logic [15:0] recv_data;  // RECV_NBYTE=2
    logic parity_err;
    ...
    // 任务：通过DUT发送数据（内部发送）
    //===========================================================================
    task automatic dut_send(input logic [7:0] data);
        begin
            $display("[DRIVE] @%0t: Trigger DUT TX, data=0x%02h", $time, data);
            @(posedge clk);
            send_data = data;
            send_en = 1;
            @(posedge clk);
    ...
    task automatic measure_baud_rate();
        ...
        begin
            ...
            // 触发一次发送
            dut_send(8'h55);  // 0101_0101，方便观察

            // 等待起始位下降沿
            @(negedge txd);
            t1 = $realtime;

            // 等待下一个下降沿（应该是数据位1->0的跳变）
            #(BIT_PERIOD_NS * 1.5);  // 跳过起始位，进入数据位
            @(negedge txd);  // 等待数据位中的下降沿
            t1 = $realtime;
            ...
            period = t2 - t1;
            $display("[TEST] Measured bit period = %0t ns", period);
            $display("[TEST] Expected bit period = %0d ns", BIT_PERIOD_NS);

            if (period > BIT_PERIOD_NS * 0.95 && period < BIT_PERIOD_NS * 1.05) begin
    ...
    initial begin
        ...
        // 将TX输出连接到RX输入（外部环回）
        rxd = txd;  // 这个会在initial块外持续驱动，需要特殊处理
